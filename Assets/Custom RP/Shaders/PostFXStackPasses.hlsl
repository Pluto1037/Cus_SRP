...
float3 ColorGrade (float3 color, bool useACES = false) {
	// color = min(color, 60.0); // 规避大值影响（由LUT的Log过程控制最值）
	color = ColorGradePostExposure(color); // 曝光
	color = ColorGradeWhiteBalance(color); // 白平衡
	color = ColorGradingContrast(color, useACES); // 对比度
	color = ColorGradeColorFilter(color); // 色彩滤镜
	color = max(color, 0.0); // 规避负值影响
	color = ColorGradeSplitToning(color, useACES); // 色调分割
	color = ColorGradingChannelMixer(color); // 通道混合器（矩阵乘法）
	color = max(color, 0.0);
	color = ColorGradingShadowsMidtonesHighlights(color, useACES); // 阴影、中间、高光
	color = ColorGradingHueShift(color); // 色相偏移
...
float3 GetColorGradedLUT (float2 uv, bool useACES = false) {
	// 由ToneMapping转到ColorGrading，进行LUT渲染（更高的颜色查找效率）
...
float3 ApplyColorGradingLUT (float3 color) {
	// 采样器切换到sampler_point_clamp后因为LUT的有限分辨率，产生了色阶效果
	return ApplyLut2D(
